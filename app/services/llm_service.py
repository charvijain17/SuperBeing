...
class OpenAIProvider:
    ...
    def generate(self, request: ChatRequest) -> ProviderResult:
        if not self.settings.openai_api_key: raise ProviderNotConfiguredError("OPENAI_API_KEY is not configured.")
...
class AnthropicProvider:
    def __init__(self, settings: Settings): self.settings = settings
    def generate(self, request: ChatRequest) -> ProviderResult:
        if not self.settings.anthropic_api_key: raise ProviderNotConfiguredError("ANTHROPIC_API_KEY is not configured.")
        import anthropic
...
class GeminiProvider:
    def __init__(self, settings: Settings): self.settings = settings
    def generate(self, request: ChatRequest) -> ProviderResult:
        if not self.settings.google_api_key: raise ProviderNotConfiguredError("GOOGLE_API_KEY is not configured.")
